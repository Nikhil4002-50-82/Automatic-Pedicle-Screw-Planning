...
# (min_diam, max_diam) - Prevents L1 from being too big and ensures L5 is stable
CLINICAL_LIMITS = {
    "L1": (4.5, 6.5),
    "L2": (4.5, 6.5),
    "L3": (5.5, 7.5),
...
wDiam = 15.0     # Priority for bone purchase
wConv = 20.0     # Forces inward "V-shape"
wDT = 5.0        # Centering in pedicle
wLen = 1.0       
wTilt = 10.0     
wMidline = 50.0  
...
def computeStableFrame(mask, affine):
    ...
    pca = PCA(n_components=3); pca.fit(coordsWorld - centroid)
...
def findEntry(center, axes, maskFloat, affine, side):
    ...
    for _ in range(100):
        ...
        if any(v<0 or v>=s-1 for v,s in zip(vox, maskFloat.shape)): break
        if map_coordinates(maskFloat, [[vox[0]],[vox[1]],[vox[2]]], order=1)[0] < 0.5: break
        p += direction * stepMM
    return p + apAxis * 1.5 
...
def evaluate(entry, direction, side, maskFloat, dist, affine, axes, centroid, maxAllowedLen, v_name):
    ...
    t = 0; minDT = 999; midlineViolation = 0
    ...
    while t < maxAllowedLen:
        ...
        if 5.0 < t < 30.0: minDT = min(minDT, dtVal)
        t += stepMM
        
    if t < minLengthMM: return None
    
    # --- Diameter Logic with Clinical Caps ---
    max_safe = (minDT * 2) - 0.5
    min_limit, max_limit = CLINICAL_LIMITS.get(v_name, (4.0, 8.5))
    
    diam = 0.0
    for d_val in globalDiameters:
        # Must physically fit AND be within clinical standard for that vertebra
...
def optimize(center, axes, side, maskFloat, dist, affine, centroid, totalDepth, v_name):
    ...
    for lrAng in angles: 
        for siAng in np.linspace(-10, 10, 11):
            direction = (axes[2] + np.tan(np.deg2rad(lrAng)) * axes[1] + np.tan(np.deg2rad(siAng)) * axes[0])
            res = evaluate(entry, direction, side, maskFloat, dist, affine, axes, centroid, maxAllowedLen, v_name)
...
def run_planner(segPath):
    ...
    for labelVal, mask in sorted(validSegments, reverse=True):
        ...
        for side, center in [("Left", lCenter), ("Right", rCenter)]:
            # Pass name to optimizer for clinical limit checks
            ...
            if res:
                ...
                print(f"{name} {side}: Diam {diam}mm, Axial {round(lrAng,1)}°, Sagittal {round(siAng,1)}°")
    return resultsList