...
def load_canonical_image(path: str) -> nib.spatialimages.SpatialImage:
    image = nib.load(path)
...
class CTLoadWorker(QObject):
    ...
    def run(self) -> None:
        try:
            image = load_canonical_image(self.path)
...
class MaskLoadWorker(QObject):
    ...
    def run(self) -> None:
        ...
        for index, path in enumerate(self.paths, start=self.start_index):
            try:
                mask_image = load_canonical_image(path)
...
class CTMaskViewer(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        status = QStatusBar()
        status.showMessage("Load a NIfTI CT to begin.")
    ...
    def _build_control_panel(self) -> QWidget:
        ...
        subtitle = QLabel(
            "Fast desktop viewer for NIfTI CT volumes and accompanying segmentation masks."
        )
        subtitle.setWordWrap(True)
        subtitle.setObjectName("panelSubtitle")

        self.load_ct_button = QPushButton("Load CT")
        ...
        layout.addWidget(self.volume_info_label)
        layout.addWidget(window_header)
    ...
    def _build_actions(self) -> None:
        load_ct_action = QAction("Load CT", self)
    ...
    def clear_viewer(self) -> None:
        ...
        self._set_loading_state("Load a NIfTI CT to begin.", busy=False)
