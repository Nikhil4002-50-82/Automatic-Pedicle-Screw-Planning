...
class SliceView(QWidget):
    ...
    def widget_to_voxel(self, x_pos, y_pos):
        ...
        x_vox = int(round(x_min + py * max(x_max - x_min, 1)))
        y_vox = int(round(y_min + (1.0 - px) * max(y_max - y_min, 1)))
        return x_vox, y_vox
    ...
    def voxel_to_widget(self, voxel):
        if self.source_shape is None:
            return 0.0, 0.0
        x_min, x_max, y_min, y_max = self.current_plane_bounds()
        x_plane, y_plane = plane_coords(self.axis, voxel)
        rect = self.image_rect()
        px = 1.0 - ((y_plane - y_min) / max(y_max - y_min, 1))
        py = (x_plane - x_min) / max(x_max - x_min, 1)
...
class GUI(QMainWindow):
    ...
    def init_ui(self):
        self.setWindowTitle("Automatic Pedicle Screw Planning System - V11.0")
        ...
        apply_icon(self.annotations_toggle_btn, "fa5s.notes-medical")
        ...
        header.addWidget(self.annotations_toggle_btn)
        ...
        annotation_layout.setContentsMargins(12, 14, 12, 12)
    ...
    def save_json(self):
        if not self.results:
            ...
            return
        file_path, _ = QFileDialog.getSaveFileName(self, "Save planning JSON", "screw_plan_v11.json", "JSON Files (*.json)")
        if not file_path:
            return
        payload = {
    ...
    def export_csv(self):
        if not self.results:
            ...
            return
        file_path, _ = QFileDialog.getSaveFileName(self, "Export planning CSV", "screw_plan_report_v11.csv", "CSV Files (*.csv)")
        if not file_path:
            return
    ...
    def save_annotations(self):
        if not self.workspace.annotations:
            ...
            return
        file_path, _ = QFileDialog.getSaveFileName(self, "Save annotations", "mpr_annotations_v11.json", "JSON Files (*.json)")
        if not file_path:
            return
        with open(file_path, "w", encoding="utf-8") as handle:
