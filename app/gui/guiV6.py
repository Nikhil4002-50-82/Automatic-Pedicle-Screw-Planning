...
from app.geometry.geometryV5 import run_planner, loadNifti, getValidLabels, computeStableFrame, computeDistance, pedicleCenters, optimize
...
class Worker(QThread):
    ...
    def __init__(self, ct_path, use_preseg):
        super().__init__()
        self.ct_path = ct_path
        self.use_preseg = use_preseg
    ...
    def run(self):
        ...
        if self.use_preseg:
            print("MODE: Using pre-segmented input (Skipping TotalSegmentator)")
            combined_path = self.ct_path
...
class GUI(QWidget):
    def __init__(self):
        ...
        self.use_preseg = False

    def initUI(self):
        self.setWindowTitle("Automatic Pedicle Screw Planning System - V6.0")
    ...
    def selectCT(self):
        ...
        if path:
            self.ct = path
            self.use_preseg = False   # 🔥 important
            self.fileLabel.setText(f"CT: {os.path.basename(path)}")
    ...
    def runPipeline(self):
        ...
        self.worker = Worker(self.ct, self.use_preseg)
