...
def make_rgb_slice(base_slice=None, seg_slice=None, opacity=0.45):
...
class Mesh3DView(QWidget):
    ...
    def render_empty(self):
        html = "<html><body style='background:#111827;color:#e5e7eb;font-family:Segoe UI;display:flex;align-items:center;justify-content:center;height:100vh;'>3D segmentation mesh will appear here</body></html>"
    ...
    def render_scene(self):
        fig = go.Figure()
        label_names = {1: "L5", 2: "L4", 3: "L3", 4: "L2", 5: "L1"}
        for mesh in self.label_meshes:
            ...
            fig.add_trace(
                go.Mesh3d(
                    ...
                    hovertemplate=f"{label_names.get(label, f'Label {label}')}<extra></extra>",
                )
            )
        fig.update_layout(
            title=dict(text="3D Colored Segmentation Mesh", x=0.5, font=dict(color="#f8fafc", size=15)),
            ...
            legend=dict(orientation="h", y=0.01, x=0.02),
        )
        with tempfile.NamedTemporaryFile(suffix=".html", delete=False) as tmp:
            fig.write_html(tmp.name, include_plotlyjs=True)
            html_path = tmp.name
        self.load_path(html_path)
    ...
    def load_path(self, html_path):
        old_path = self.html_path
...
class FourViewWorkspace(QWidget):
    ...
    def init_ui(self):
        ...
        for row, col, axis in [(0, 0, "Axial"), (0, 1, "Coronal"), (1, 0, "Sagittal")]:
            ...
            header = QHBoxLayout()
            title = QLabel(axis)
            title.setStyleSheet("font-size: 14px; font-weight: 900; color: #172554;")
            expand_btn = QPushButton("⛶")
            expand_btn.setToolTip(f"Expand {axis} view")
            expand_btn.setFixedSize(34, 30)
        ...
        mesh_layout.addLayout(mesh_header)
    ...
    def update_mpr_views(self):
        ...
        for axis, view in self.views.items():
            ...
            rgb = make_rgb_slice(base, seg, self.opacity)
    ...
    def set_view_pixmap(self, view, rgb, axis, shape):
        ...
        pixmap = self.draw_overlays(pixmap, axis, shape)
        view.setPixmap(
            pixmap.scaled(
                view.size(),
...
class GUI(QMainWindow):
    ...
    def init_ui(self):
        self.setWindowTitle("Automatic Pedicle Screw Planning System - V9.0")
        ...
        self.annotations_toggle_btn = QPushButton("Annotations")
        self.annotations_toggle_btn.setCheckable(True)
        self.annotations_toggle_btn.clicked.connect(self.toggle_annotations)
        ...
        toolbar.addWidget(self.actions_toggle_btn)
        toolbar.addWidget(self.annotations_toggle_btn)
        ...
        self.annotation_panel = QGroupBox("Annotations")
        self.annotation_panel.setObjectName("ActionsPanel")
        ...
        self.annotation_panel.setFixedWidth(330)
        self.annotation_panel.hide()
        workspace_layout.addWidget(self.annotation_panel)
    ...
    def apply_style(self):
        self.setStyleSheet(
            ...
            "QGroupBox#ActionsPanel { background: #f8fafc; border: 1px solid #bfdbfe; border-radius: 6px; }"
            "QLabel#PanelTitle { color: #172554; font-size: 13px; font-weight: 900; padding-top: 8px; }"
    ...
    def toggle_annotations(self):
        visible = self.annotations_toggle_btn.isChecked()
        self.annotation_panel.setVisible(visible)
    ...
    def load_seg_from_path(self, path):
        ...
        try:
            ...
            self.workspace.set_mesh(build_label_meshes(self.seg_data, nii.affine))
    ...
    def refresh_annotation_list(self):
        ...
        for annotation in self.workspace.annotations:
            self.add_annotation_row(annotation)
    ...
    def save_json(self):
        if not self.results:
            ...
            return
        file_path, _ = QFileDialog.getSaveFileName(self, "Save planning JSON", "screw_plan_v9.json", "JSON Files (*.json)")
        if not file_path:
            return
        payload = {
    ...
    def export_csv(self):
        if not self.results:
            ...
            return
        file_path, _ = QFileDialog.getSaveFileName(self, "Export planning CSV", "screw_plan_report_v9.csv", "CSV Files (*.csv)")
        if not file_path:
            return
    ...
    def save_annotations(self):
        if not self.workspace.annotations:
            ...
            return
        file_path, _ = QFileDialog.getSaveFileName(self, "Save annotations", "mpr_annotations_v9.json", "JSON Files (*.json)")
        if not file_path:
            return
        with open(file_path, "w", encoding="utf-8") as handle:
