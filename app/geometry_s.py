...
def findEntry_vectorized(center, axes, maskFloat, affine, side):
    """Vectorized entry search. FIXED: Axis alignment for nibabel.apply_affine."""
    siAxis, lrAxis, apAxis = axes
    ...
    # nibabel.affines.apply_affine expects (N, 3)
...
def evaluate_vectorized(entry, direction, side, maskFloat, dist, affine, axes, centroid, maxAllowedLen, v_name):
    """Vectorized ray evaluation. FIXED: Axis alignment for nibabel.apply_affine."""
    ...
    # Apply affine: expects (N, 3) -> returns (N, 3)
    voxels = nib.affines.apply_affine(invAff, points.T)
    
    shape = np.array(maskFloat.shape)
    valid_bounds = np.all((voxels >= 0) & (voxels < shape - 1), axis=1)
    
    # Sample mask & dist: expects (3, N)
    mask_vals = map_coordinates(maskFloat, voxels.T, order=1)
...
def process_single_vertebra(labelVal, mask, name, affine, spacing, log_queue=None):
    """Process exactly one vertebra. Aggressively frees memory when done."""
...
def run_planner(segPath, log_queue=None):
    ...
    _log("\n[INFO] Processing vertebrae sequentially (memory-safe mode)...", log_queue)
    t = time.perf_counter()
    
    for labelVal, mask in sorted(validSegments, reverse=True):
        name = labelMap.get(labelVal, str(labelVal))
        res = process_single_vertebra(labelVal, mask, name, affine, spacing, log_queue=log_queue)
